...
VMD_CONFIG = {
    "5m": {
        "Tran": {"K": 3, "centers_hz": [10.0, 40.0, 100.0]},
        "Vert": {"K": 3, "centers_hz": [10.0, 40.0, 100.0]},
        "Long": {"K": 3, "centers_hz": [10.0, 40.0, 100.0]},
    },
    "10m": {
        "Tran": {"K": 3, "centers_hz": [10.0, 40.0, 100.0]},
        "Vert": {"K": 3, "centers_hz": [10.0, 40.0, 100.0]},
...
以上数值只是可执行的示例默认值，用户可直接编辑。每个 `centers_hz` 必须
明确列出全部 K 个初始中心；程序不自动添加 0 Hz 残余模态。若需要 0 Hz
初始模态，用户应把 `0.0` 明确写入该列表并相应计入 K。

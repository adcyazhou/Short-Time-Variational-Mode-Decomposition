...
def test_original_buffer_and_vmd_sources_are_verbatim():
...
def test_original_vmd_returns_single_channel_shapes():
    namespace = notebook_namespace()
    t = np.arange(64) / 128
...
def test_plotting_and_saving_vmd_results(tmp_path):
    namespace = notebook_namespace()
    ...
    summary = namespace["summarize_vmd_result"](
        values.reshape(1, -1), 128, raw
    )
    namespace["PLOT_MAX_HZ"] = 64.0
    figures = namespace["plot_vmd_results"](waveform, summary)
    assert set(figures) == {
        "input_modes",
        "center_frequencies",
    ...
    namespace["save_vmd_results"](tmp_path, waveform, summary, figures)
    assert len(list(tmp_path.glob("*.png"))) == 4
    assert (tmp_path / "vmd_single_waveform_results.npz").is_file()
